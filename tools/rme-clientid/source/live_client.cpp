...
void LiveClient::tryConnect(boost::asio::ip::tcp::resolver::results_type::iterator endpoint_iterator)
{
	...
	logMessage("Joining server " + endpoint_iterator->endpoint().address().to_string() + ":" + std::to_string(endpoint_iterator->endpoint().port()) + "...");

	boost::asio::async_connect(*socket, endpoint_iterator, boost::asio::ip::tcp::resolver::results_type::iterator(), [this](boost::system::error_code error, boost::asio::ip::tcp::resolver::results_type::iterator endpoint_iterator) -> void
	{
		if(!socket->is_open()) {
