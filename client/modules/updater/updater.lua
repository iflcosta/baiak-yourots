...
--  else
    g_window.setMinimumSize({width = 802, height = 450})
--  end
...
local function downloadFiles(url, files, index, retries, doneCallback)
  ...
  updaterWindow.progressBg.mainProgress:setPercent(0)

  local totalDownloadedBytes = 0
  local lastClock = g_clock.millis()
  ...
  httpOperationId =
    ...
    function(progress, bytesDownloaded)
      local currentClock = g_clock.millis()
      local timeElapsed = (currentClock - lastClock) / 1000
      if timeElapsed < 1 then
          updaterWindow.progressBg.mainProgress:setPercent(progress)
          return
      end

      local speedBps = bytesDownloaded / timeElapsed
      lastClock = currentClock
      updaterWindow.progressBg.mainProgress:setPercent(progress)
      updaterWindow.progressBg.downloadStatus:setText(string.format("%.2fMBs", (speedBps / 1024)))
    end
  )
end
...
local function updateFiles(data, keepCurrentFiles)
  ...
  if #toUpdate == 0 then -- nothing to update
    updaterWindow.progressBg.mainProgress:setPercent(100)
  ...
  updaterWindow.version:setText(tr("Updating %i files", #toUpdate))
  updaterWindow.progressBg.mainProgress:setPercent(0)
  downloadFiles(
    data["url"],
    toUpdate,
