...
function LoadTibiaMap()
  ...
  local function asyncLoadImage(file)
    ...
    if #pos >= 3 then
      ...
      if x and y and z then
        g_minimap.loadImage('/minimap/' .. file, { x = x * 256, y = y * 256, z = z }, 1.0)
      end
    end
  end
  ...
  local function loadVisibleImages()
    local files = g_resources.listDirectoryFiles("/minimap", false)
    g_logger.info("DEBUG: listDirectoryFiles(/minimap, false) returned " .. tostring(#files) .. " entries")
