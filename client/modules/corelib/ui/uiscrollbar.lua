...
local function calcValues(self)
  ...
  if self.orientation == 'vertical' then
    pxrange = self:getHeight()
              - decrementButton:getHeight() - decrementButton:getMarginTop() - decrementButton:getMarginBottom()
    ...
    pxrange = self:getWidth()
              - decrementButton:getWidth() - decrementButton:getMarginLeft() - decrementButton:getMarginRight()
  ...
  local minSize = g_app.isMobile() and 24 or 12
...
function UIScrollBar:onSetup()
  ...
  g_mouse.bindAutoPress(self:getChildById('decrementButton'), function()
    if self.orientation == 'vertical' then
      ...
      if UIScrollBar.onDecrement then
        self:onDecrement()
      end
    end
  ...
  self.onClick = function() self:onClickSlider() end
  updateSlider(self)
end
...
function UIScrollBar:onClickSlider(slider)
  ...
  if self.orientation == 'vertical' then
    self:setSliderPos(slider, slider:getPosition(), {y = mousePos.y - slider:getPosition().y, x = 0})
  else
    self:setSliderPos(slider, slider:getPosition(), {x = mousePos.x - slider:getPosition().x, y = 0})
  end
end
...
function UIScrollBar:setDefaultScroll() self.defaultSlider = true end
...
function UIScrollBar:setVirtualChilds(value) self.virtualChilds = value updateSlider(self) end
function UIScrollBar:setVisibleItems(value) self.visibleItems = value end -- mest be called before setVirtualChilds 
