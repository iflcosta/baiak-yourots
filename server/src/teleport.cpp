...
void Teleport::addThing(int32_t, Thing* thing)
{
	...
	if (Creature* creature = thing->getCreature()) {
		...
		if (effect != CONST_ME_NONE) {
			g_game.addMagicEffect(origPos, effect, creature->getInstanceID());
			g_game.addMagicEffect(destTile->getPosition(), effect, creature->getInstanceID());
		}
	} else if (Item* item = thing->getItem()) {
		if (effect != CONST_ME_NONE) {
			g_game.addMagicEffect(destTile->getPosition(), effect, item->getInstanceID());
			g_game.addMagicEffect(item->getPosition(), effect, item->getInstanceID());
		}
