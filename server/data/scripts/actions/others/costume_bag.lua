local config = {
	[653] = {
		"orc warrior", "pirate cutthroat", "dworc voodoomaster", "dwarf guard",
  "minotaur mage"
	}, -- common
	[655] = {"serpent spawn", "demon", "juggernaut", "behemoth", "ashmunrah"}, -- deluxe
	[654] = {
		"quara hydromancer", "diabolic imp", "banshee", "frost giant", "lich"
	} -- uncommon
}

local costumeBag = Action()

function costumeBag.onUse(player, item, fromPosition, target, toPosition,
                          isHotkey)
...
for k, v in pairs(config) do costumeBag:id(k) end
costumeBag:register()
