...
function voodooDoll.onUse(player, item, fromPosition, target, toPosition,
                          isHotkey)
	if target.itemid ~= 1 or not target:isPlayer() then return false end
	...
	if math.random(100) <= 5 then
		...
		text =
			"You concentrate on your victim, hit the needle in the doll.......but nothing happens."
	end
